    ...
    public static function displayProjects(array $projects): string
    {
        ...
        foreach ($projects as $project) {
            if ($project->deadline < $today) {
                $output .= "<a href='project.php' class='hover:underline rounded-lg border border-red-600 p-4 py-6 text-4xl font-bold w-full bg-red-300'>
                        {$project->name}
                        </a>";
            } else {
                $output .= "<a href='project.php' class='hover:underline rounded-lg border p-4 py-6 text-4xl font-bold w-full bg-slate-300'>
                        {$project->name}
                        </a>";
            }
        }
